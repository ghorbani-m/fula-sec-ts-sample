...
export class FullaDID implements IFullaDID {
    ...
    async create () {
        ...
        let EthHdWallet = EthHdWallet.fromMnemonic(this.mnemonic)
        this.privateKey = EthHdWallet.derive(`m/44'/60'/0'/0/0`).getPrivateKey().toString('hex')
    ...
    async importMnemonic (mnemonic: string) {
        let EthHdWallet = EthHdWallet.fromMnemonic(mnemonic);
        this.privateKey = EthHdWallet.derive(`m/44'/60'/0'/0/0`).getPrivateKey().toString('hex')
        this.authDID = await this.didProvider();
        return {
            privateKey: this.privateKey,
